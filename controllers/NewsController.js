...
class NewsController {
    ...
    async getMaincontent(req, res) {
        try {
            var con = mysql2.createConnection(configDB);// Tạo connection
            var main_contents = await new Promise((resolve, rejects) => {
                con.query('SELECT * FROM main_content', function (err, result) {
                    if (err) rejects(err);
                    resolve(result)
                })

            })
            res.status(200).send(main_contents);
